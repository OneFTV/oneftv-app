...
interface DarkRoundRobinViewProps {
  games: BracketGame[];
}
...
export default function DarkRoundRobinView({ games }: DarkRoundRobinViewProps) {
  ...
  const getResult = (
    ...
    if (rowTeam === colTeam) {
      return { display: '', className: 'bg-dark-bg' };
    }
    ...
    if (direct || reverse) {
      return { display: '-', className: 'text-gray-600' };
    }

    return { display: '', className: 'text-gray-700' };
  };
    ...
    <div className="space-y-8">
      ...
      <div>
        <div className="flex items-center gap-3 mb-5">
          ...
          <h4 className="text-lg font-bold text-gray-100">Results Matrix</h4>
        </div>
          ...
          <table className="min-w-full text-xs border-collapse">
            ...
            <tbody>
              {teams.map((rowTeam, rowIdx) => (
                <tr key={rowTeam} className={rowIdx % 2 === 0 ? 'bg-dark-surface' : 'bg-dark-elevated'}>
                  <td className="sticky left-0 z-10 bg-inherit px-3 py-2 font-semibold text-gray-200 border-r border-dark-border whitespace-nowrap text-xs">
                    {rowTeam}
                  </td>
                  {teams.map((colTeam) => {
                    ...
                    return (
                      <td
                        key={colTeam}
                        className={`px-2 py-2 text-center border border-dark-border font-mono text-xs min-w-[52px] ${result.className}`}
                      >
                        {result.display}
                      </td>
                    );
      ...
      <div>
        <div className="flex items-center gap-3 mb-5">
          ...
          <h4 className="text-lg font-bold text-gray-100">Standings</h4>
        </div>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {sortedTeams.map((team, idx) => {
                ...
                return (
                  <tr key={team} className="border-t border-dark-border hover:bg-dark-elevated transition-colors bg-dark-surface">
                    <td className="px-4 py-2.5 text-gray-500 font-medium">{idx + 1}</td>
                    <td className="px-3 py-2.5 font-semibold text-gray-200">{team}</td>
                    <td className="text-center px-2 py-2.5 text-green-400 font-medium">{s.wins}</td>
                    <td className="text-center px-2 py-2.5 text-red-400 font-medium">{s.losses}</td>
                    <td className="text-center px-2 py-2.5 text-gray-400">{s.pf}</td>
                    <td className="text-center px-2 py-2.5 text-gray-400">{s.pa}</td>
