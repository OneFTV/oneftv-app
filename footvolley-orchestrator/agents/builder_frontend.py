"""Frontend builder agent: React/UI tasks on READY+frontend tasks."""
...
def _load_prompt_template():
    ...
    return config['builder_frontend']['prompt_template']


def run(db, task):
    """Build frontend code for a READY task.
    ...
    task_id = task['id']
    logger.info(f"Frontend builder: task {task_id}: {task['title']}")

    # Acquire lease
    lease_id = begin_execution(db, task_id, 'builder_frontend')
    if not lease_id:
        log_agent(db, 'builder_frontend', 'lease_failed',
    ...
    try:
        ...
        if not worktree_path:
            log_agent(db, 'builder_frontend', 'worktree_failed',
        ...
        if not response:
            log_agent(db, 'builder_frontend', 'build_failed',
