...
async function main() {
  ...
  // Update TournamentPlayer records with computed W/L stats (pointDiff only)
  let updatedCount = 0;
  ...
  // ═══════════════════════════════════════════════════════════
  // ASSIGN NFA RANKING POINTS BY FINAL PLACEMENT
  // ═══════════════════════════════════════════════════════════
  console.log('Assigning NFA ranking points by final placement...');

  // NFA Points table by placement (team-based, both players get same points)
  const NFA_POINTS: Record<number, number> = {
    1: 1000,
    2: 750,
    3: 500,
    4: 350,
  ...
  // Helper to assign points to a team (both players)
  async function assignNfaPoints(team: TeamDef, categoryId: string, placement: number) {
    const pts = NFA_POINTS[placement] || 25;
    for (const name of [team.p1, team.p2]) {
      await prisma.tournamentPlayer.updateMany({
        where: { tournamentId: tournament.id, userId: uid(name), categoryId },
        data: { points: pts },
      });
    }
  }

  // ── Open Division 1 (32 teams) ──
  // 1st: Moises Davalos / Ivan Davalos (won final)
  // 2nd: Fernando Chorei / Billy Oliveira (lost final)
  // 3rd: Pedro Ivo / Felipe Carbonaro (won 3rd place match)
  // 4th: Thiago Mixirica / Cesar Fiorio (lost 3rd place match)
  // 5-8: Semi-final losers from L6 + W4 losers = Bruno BR7/Victor Martinez, Mazo Soares/Wesley Oliveira, Afonso Pinheiro/Pedro Kassem, Diego Tavares/Yuri Ribeiro
  await assignNfaPoints(t('Moises Davalos', 'Ivan Davalos'), catOpenDiv1.id, 1);
  await assignNfaPoints(t('Fernando Chorei', 'Billy Oliveira'), catOpenDiv1.id, 2);
  await assignNfaPoints(t('Pedro Ivo', 'Felipe Carbonaro'), catOpenDiv1.id, 3);
  await assignNfaPoints(t('Thiago Mixirica', 'Cesar Fiorio'), catOpenDiv1.id, 4);
  await assignNfaPoints(t('Bruno BR7', 'Victor Martinez'), catOpenDiv1.id, 5);
  await assignNfaPoints(t('Mazo Soares', 'Wesley Oliveira'), catOpenDiv1.id, 6);
  await assignNfaPoints(t('Afonso Pinheiro', 'Pedro Kassem'), catOpenDiv1.id, 7);
  await assignNfaPoints(t('Diego Tavares', 'Yuri Ribeiro'), catOpenDiv1.id, 8);
  // 9-16: L4/L3/L2 losers
  await assignNfaPoints(t('Marcos Chantel', 'Anderson Silva'), catOpenDiv1.id, 9);
  await assignNfaPoints(t('Zuca Palladino', 'Lucas Silva'), catOpenDiv1.id, 10);
  await assignNfaPoints(t('Bernardo Avila', 'Tuio Silveira'), catOpenDiv1.id, 11);
  await assignNfaPoints(t('Thiago Cunha', 'Lucca Toledo'), catOpenDiv1.id, 12);
  await assignNfaPoints(t('Fabricio Barancoski', 'Piu Montemor'), catOpenDiv1.id, 13);
  await assignNfaPoints(t('Guiga Muller', 'Breno Soares'), catOpenDiv1.id, 14);
  await assignNfaPoints(t('Pedro Espindola', 'Pedro Galimberti'), catOpenDiv1.id, 15);
  await assignNfaPoints(t('Junior Baiano', 'Michael Douglas'), catOpenDiv1.id, 16);
  // 17-24: L2 losers
  await assignNfaPoints(t('Felipe Netto', 'Bruno Vieira'), catOpenDiv1.id, 17);
  await assignNfaPoints(t('Roberto Asmar', 'Ademir Cabral'), catOpenDiv1.id, 18);
  await assignNfaPoints(t('Gustavo Guimaraes', 'Thiago Guimaraes'), catOpenDiv1.id, 19);
  await assignNfaPoints(t('Henrique Oliveira', 'Carlos Iwata'), catOpenDiv1.id, 20);
  await assignNfaPoints(t('Matheus Paes', 'Fe Schneider'), catOpenDiv1.id, 21);
  await assignNfaPoints(t('Thiago Platz', 'Gilberto Camillato'), catOpenDiv1.id, 22);
  await assignNfaPoints(t('Rafa Barao', 'Igor Martins'), catOpenDiv1.id, 23);
  await assignNfaPoints(t('Gabriel Souza', 'Leozinho Gomes'), catOpenDiv1.id, 24);
  // 25-32: L1 losers (eliminated in round 2)
  await assignNfaPoints(t('Janser Pinho', 'Joao Gabriel Freitas'), catOpenDiv1.id, 25);
  await assignNfaPoints(t('Lucas Moraes', 'Vladimir Silveira'), catOpenDiv1.id, 26);
  await assignNfaPoints(t('Manoel Carneiro', 'Bruno Marcelo'), catOpenDiv1.id, 27);
  await assignNfaPoints(t('William Jorge', 'Matheus Eller'), catOpenDiv1.id, 28);
  await assignNfaPoints(t('Ricardo Pavei', 'Dalan Telles'), catOpenDiv1.id, 29);
  await assignNfaPoints(t('Patricia Rodriguez', 'Rogerio Teixeira'), catOpenDiv1.id, 30);
  await assignNfaPoints(t('Breno da Mata', 'Luan Maia'), catOpenDiv1.id, 31);
  await assignNfaPoints(t('Gabriela Allen-Vieira', 'Fabiola Zanella'), catOpenDiv1.id, 32);

  // ── Open Division 2 (8 teams) ──
  await assignNfaPoints(t('Gabriel Souza', 'Leozinho Gomes'), catOpenDiv2.id, 1);
  await assignNfaPoints(t('Zuca Palladino', 'Lucas Silva'), catOpenDiv2.id, 2);
  await assignNfaPoints(t('Pedro Espindola', 'Pedro Galimberti'), catOpenDiv2.id, 3);
  await assignNfaPoints(t('Thiago Cunha', 'Lucca Toledo'), catOpenDiv2.id, 4);
  await assignNfaPoints(t('Marcos Chantel', 'Anderson Silva'), catOpenDiv2.id, 5);
  await assignNfaPoints(t('Bernardo Avila', 'Tuio Silveira'), catOpenDiv2.id, 6);
  await assignNfaPoints(t('Fabricio Barancoski', 'Piu Montemor'), catOpenDiv2.id, 7);
  await assignNfaPoints(t('Junior Baiano', 'Michael Douglas'), catOpenDiv2.id, 8);

  // ── Open Division 3 (16 teams) ──
  await assignNfaPoints(t('Rafa Barao', 'Igor Martins'), catOpenDiv3.id, 1);
  await assignNfaPoints(t('Felipe Netto', 'Bruno Vieira'), catOpenDiv3.id, 2);
  await assignNfaPoints(t('Guiga Muller', 'Breno Soares'), catOpenDiv3.id, 3);
  await assignNfaPoints(t('Matheus Paes', 'Fe Schneider'), catOpenDiv3.id, 4);
  await assignNfaPoints(t('Roberto Asmar', 'Ademir Cabral'), catOpenDiv3.id, 5);
  await assignNfaPoints(t('William Jorge', 'Matheus Eller'), catOpenDiv3.id, 6);
  await assignNfaPoints(t('Lucas Moraes', 'Vladimir Silveira'), catOpenDiv3.id, 7);
  await assignNfaPoints(t('Henrique Oliveira', 'Carlos Iwata'), catOpenDiv3.id, 8);
  await assignNfaPoints(t('Ricardo Pavei', 'Dalan Telles'), catOpenDiv3.id, 9);
  await assignNfaPoints(t('Patricia Rodriguez', 'Rogerio Teixeira'), catOpenDiv3.id, 10);
  await assignNfaPoints(t('Manoel Carneiro', 'Bruno Marcelo'), catOpenDiv3.id, 11);
  await assignNfaPoints(t('Thiago Platz', 'Gilberto Camillato'), catOpenDiv3.id, 12);
  await assignNfaPoints(t('Janser Pinho', 'Joao Gabriel Freitas'), catOpenDiv3.id, 13);
  await assignNfaPoints(t('Gustavo Guimaraes', 'Thiago Guimaraes'), catOpenDiv3.id, 14);
  await assignNfaPoints(t('Breno da Mata', 'Luan Maia'), catOpenDiv3.id, 15);
  await assignNfaPoints(t('Gabriela Allen-Vieira', 'Fabiola Zanella'), catOpenDiv3.id, 16);

  // ── Women's Division (8 teams) ──
  await assignNfaPoints(t('Karol Muniz', 'Jarede Cesar'), catWomens.id, 1);
  await assignNfaPoints(t('Gabriela Allen-Vieira', 'Fabiola Zanella'), catWomens.id, 2);
  await assignNfaPoints(t('Juliana Benn', 'Malu Costa'), catWomens.id, 3);
  await assignNfaPoints(t('Fiorella Pelegrini', 'Gabi Macedo'), catWomens.id, 4);
  await assignNfaPoints(t('Tata Moreira', 'Patricia Rodriguez'), catWomens.id, 5);
  await assignNfaPoints(t('Kaitlyn Brunworth', 'Gaby Coelho'), catWomens.id, 6);
  await assignNfaPoints(t('Ana Paula Khouri', 'Felicia Cunha'), catWomens.id, 7);
  await assignNfaPoints(t('Duda Souza', 'Julia Novaes'), catWomens.id, 8);

  // ── Master Division (8 teams) ──
  await assignNfaPoints(t('Alex Batuta', 'Junior Pereira'), catMaster.id, 1);
  await assignNfaPoints(t('Sandro de Sa', 'Herivelto Nogueira'), catMaster.id, 2);
  await assignNfaPoints(t('Jairo Braga', 'Jefferson David'), catMaster.id, 3);
  await assignNfaPoints(t('Alexandre Vieira', 'Socrates Demetrius'), catMaster.id, 4);
  await assignNfaPoints(t('Giovanni Silva', 'Rivelino Oliveira'), catMaster.id, 5);
  await assignNfaPoints(t('Araken Fernandes', 'Heron Tavares'), catMaster.id, 6);
  await assignNfaPoints(t('Carlos Montemor', 'Marco Muricy'), catMaster.id, 7);
  await assignNfaPoints(t('Rafael Palhinha', 'Marcos Araujo'), catMaster.id, 8);

  // ── Beginners Division (16 teams) ──
  await assignNfaPoints(t('Leo Andreucci', 'Ricardo Pereira'), catBeginners.id, 1);
  await assignNfaPoints(t('Breno Gama', 'Luis Quintella'), catBeginners.id, 2);
  await assignNfaPoints(t('Erick Wiedemann', 'Richard de Souza'), catBeginners.id, 3);
  await assignNfaPoints(t('Marcos Souza', 'Daniel Cortes'), catBeginners.id, 4);
  await assignNfaPoints(t('Arthur Mesquita', 'Henrique Leandro'), catBeginners.id, 5);
  await assignNfaPoints(t('Alexandre Souza', 'Ceni Azevedo'), catBeginners.id, 6);
  await assignNfaPoints(t('Alexandre Vieira', 'Socrates Demetrius'), catBeginners.id, 7);
  await assignNfaPoints(t('Bruna Leao', 'Daniela Diaz'), catBeginners.id, 8);
  await assignNfaPoints(t('Robin Sibalin', 'Karoly Csak'), catBeginners.id, 9);
  await assignNfaPoints(t('Marco Muricy', 'Rafael Barros'), catBeginners.id, 10);
  await assignNfaPoints(t('Joao Paulo Passos', 'Kaique Fagundes'), catBeginners.id, 11);
  await assignNfaPoints(t('Marcelo Oliveira', 'Flavio Silva'), catBeginners.id, 12);
  await assignNfaPoints(t('Stella Gress', 'Bryan Martinho'), catBeginners.id, 13);
  await assignNfaPoints(t('Chandler Wilder', 'Izhak Revah'), catBeginners.id, 14);
  await assignNfaPoints(t('Nino Ferreira', 'Kris Ramos'), catBeginners.id, 15);
  await assignNfaPoints(t('Marcel Gomes', 'Filipe Palacios'), catBeginners.id, 16);

  console.log('NFA ranking points assigned!');
